...
1. **Background**: the existing system relevant to this change. The reader's prior knowledge is unknown: deep background for beginners (marked skippable), then a narrow background directly relevant to the change. Use the project's glossary terms where a `CONTEXT.md` exists.

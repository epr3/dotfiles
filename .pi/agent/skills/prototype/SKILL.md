...
Then the code: the real implementation is a rewrite under production constraints, informed by the prototype rather than promoted from it. Capture the prototype itself as a **primary source**: commit it to a throwaway branch off main and leave a pointer to that branch from the ticket; main keeps only the validated decision. Never ship the shell; never promote the prototype.

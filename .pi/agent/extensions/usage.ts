...
/** Timeout for CLI exec commands (ms). */
const CLI_EXEC_TIMEOUT_MS = 10_000;
/** Milliseconds per minute, used for time calculations. */
...
/** Wraps fetch with an AbortController timeout (default 5 s). */
...
async function fetchGeminiStatus(): Promise<ProviderStatus> {
	try {
		...
		const res = await fetchWithTimeout("https://www.google.com/appsstatus/dashboard/incidents.json");
		if (!res.ok) return { indicator: "unknown" };
...
async function fetchCopilotUsage(): Promise<UsageSnapshot> {
	...
	try {
		...
		if (data.quota_snapshots?.premium_interactions) {
			...
			const usedPercent = Math.max(0, 100 - (premiumInteractions.percent_remaining || 0));
			windows.push({
				label: `Premium`,
				usedPercent,
...
async function fetchKiroUsage(): Promise<UsageSnapshot> {
	...
	try {
		// Check if logged in
		try {
			execSync("kiro-cli whoami", { encoding: "utf-8", timeout: FETCH_TIMEOUT_MS });
		} catch {
...
function loadNanoGptApiKey(): string | undefined {
	const authPath = path.join(os.homedir(), ".pi", "agent", "auth.json");
	try {
		if (fs.existsSync(authPath)) {
			const data = JSON.parse(fs.readFileSync(authPath, "utf-8"));
			if (data["nanogpt"]?.access) return data["nanogpt"].access;
		}
	} catch {}

	if (process.env.NANOGPT_API_KEY) return process.env.NANOGPT_API_KEY;

	return undefined;
}

async function fetchNanoGptUsage(): Promise<UsageSnapshot> {
	const apiKey = loadNanoGptApiKey();
	if (!apiKey) {
		return { provider: "nanogpt", displayName: "nanoGPT", windows: [], error: "No API key" };
	}
	...
	try {
		const [balanceRes, usageRes] = await Promise.all([
			fetchWithTimeout("https://nano-gpt.com/api/check-balance", { method: "POST", headers }),
			fetchWithTimeout("https://nano-gpt.com/api/subscription/v1/usage", { headers }),
		]);

		// --- balance → plan string ---
		// Shape: { usd_balance: string, nano_balance: string, nanoDepositAddress: string }
		let plan: string | undefined;
		if (balanceRes.ok) {
			const b = await balanceRes.json() as any;
			if (b.usd_balance !== undefined) {
				const usd = parseFloat(b.usd_balance);
				if (!isNaN(usd)) plan = `$${usd.toFixed(2)}`;
			}
		}

		// --- subscription usage → windows ---
		// Shape: { weeklyInputTokens: {percentUsed, resetAt(ms)} | null,
		//          dailyImages: {percentUsed, resetAt(ms)} | null,
		//          dailyInputTokens: {percentUsed, resetAt(ms)} | null, ... }
		const windows: RateWindow[] = [];
		if (usageRes.ok) {
			const u = await usageRes.json() as any;

			const windowDefs: Array<{ key: string; label: string }> = [
				{ key: "weeklyInputTokens", label: "Tokens (weekly)" },
				{ key: "dailyInputTokens", label: "Tokens (daily)" },
				{ key: "dailyImages", label: "Images (daily)" },
			];
			...
			for (const { key, label } of windowDefs) {
				...
				if (w != null && typeof w === "object") {
					windows.push({
						label,
						usedPercent: Math.min(100, (w.percentUsed ?? 0) * 100),
						resetDescription: w.resetAt ? formatReset(new Date(w.resetAt)) : undefined,
					});
				}
			}
		}

		return { provider: "nanogpt", displayName: "nanoGPT", windows, plan };
	} catch (e) {
		return { provider: "nanogpt", displayName: "nanoGPT", windows: [], error: String(e) };
	}
}
...
class UsageComponent {
	...
	private async load() {
		...
		const [claude, copilot, gemini, codex, antigravity, kiro, zai, nanogpt, claudeStatus, copilotStatus, geminiStatus, codexStatus] = await Promise.all([
			...
			timeout(fetchNanoGptUsage(), 6000, { provider: "nanogpt", displayName: "nanoGPT", windows: [], error: "Timeout" }),
		...
		const allUsages = [claude, copilot, gemini, codex, antigravity, kiro, zai, nanogpt];
